...
export interface ConcaveCarouselProps {
  ...
  /** Mobile-first default for visible items. @default 3 */
  visibleItems?: number
  /** Mobile-first default for the center item's max height in pixels. @default 250 */
  maxHeight?: number
  /** Mobile-first default for the outer items' min height in pixels. @default 100 */
  minHeight?: number
  ...
  stepDuration?: number
  /** How aggressively the carousel responds to mouse/wheel drag velocity. @default 0.003 */
...
export function ConcaveCarousel({
  ...
  useGSAP(
    () => {
      ...
      // WCAG Safety: Detects motion sensitivity and aggressively disables infinite scrolling loops
          ...
          for (const bp of bps) {
            if (windowWidth >= bp) {
              ...
              if (breakpoints[bp].maxHeight)
                activeMaxH = breakpoints[bp].maxHeight
              if (breakpoints[bp].minHeight)
                activeMinH = breakpoints[bp].minHeight
            }
          }
        }
      ...
      const scheduleNextStep = () => {
        ...
        // Disable stepping if user prefers reduced motion
      ...
      const observer = Observer.create({
        ...
        onDrag: (e) => {
          // Touch screens emit larger drag values, Observer normalizes this dynamically
      ...
      const update = () => {
        ...
        for (let i = 0; i < totalItems; i++) {
          ...
          const scale =
            activeMinScale + (1 - activeMinScale) * (normalizedD * normalizedD)
