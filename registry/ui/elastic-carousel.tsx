...
> {
  ...
  /** Distance between cards in the 3D space. @default 1.2 */
  gapMultiplier?: number
  /** Controls the final cylinder size when fully bent. @default 1.2 */
  radiusMultiplier?: number

  // --- Unified Physics ---
  /** How heavily the scroll wheel/drag affects movement. @default 0.005 */
  scrollSensitivity?: number
  /** How buttery smooth the momentum is (0.01 to 1). @default 0.08 */
  lerpFactor?: number

  // --- Unified Geometry ---
  /** How aggressively the flat gallery bends into a 3D cylinder on scroll. @default 0.5 */
  flexMultiplier?: number

  // --- Unified Shaders ---
  /** Intensity of the internal texture slide. @default 0.08 */
  parallaxIntensity?: number
  /** Intensity of the color split on scroll. @default 0.003 */
  chromaticAberrationIntensity?: number
  /** How much the cards darken as they move to the back. @default 0.4 */
...
// --------------------------------------------------------
// GLSL SHADERS (Unified Variables & SDF Corners)
// --------------------------------------------------------
...
void main() {
  ...
  // The vertex flex only happens when velocity is high AND it is wrapping
...
void main() {
  ...
  // 4. Atmospheric Perspective (Dimming & Theme-Agnostic Alpha Fading)
  ...
  // Alpha gracefully fades into the bg-background theme
...
function ElasticScene({
  ...
  // This guarantees the cards never clip out of the vertical camera frustum
    ...
    return textures.map((texture) => {
      ...
      return new THREE.ShaderMaterial({
        ...
        uniforms: {
          ...
          // Accessibility: Strip physics if user prefers reduced motion
        ...
        transparent: true,
        depthWrite: false, // Ensures alpha blending works perfectly
      })
    })
  }, [
    textures,
    ...
    if (groupRef.current) {
      groupRef.current.children.forEach(
        (mesh: THREE.Mesh | any, reversedIndex) => {
          ...
          // Seamless Physics Interpolation
          ...
          // GUARANTEED Z-SORTING for Alpha Blending
          ...
          material.uniforms.uDepth.value = THREE.MathUtils.lerp(
            1.0,
            Math.cos(angle),
            state.bend
          )
