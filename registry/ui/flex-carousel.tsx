...
export interface FlexCarouselProps extends Omit<
...
> {
  ...
  /** Space between cards in relation to viewport width. @default 0.36 */
  ...
  lerpFactor?: number
  /** How much the cards bow/bend physically when moving fast. @default 0.25 */
  flexMultiplier?: number
  /** How much the cards rotate horizontally to face the center. @default 0.02 */
  rotationMultiplier?: number
  /** Intensity of the parallax effect inside the image frames. @default 0.05 */
  parallaxIntensity?: number
  /** The intensity of the kinetic RGB color splitting. @default 0.005 */
  chromaticAberrationIntensity?: number
  /** How intensely the edges dim based on curvature/movement. @default 0.015 */
  dimmingMultiplier?: number
  /** SDF Corner Radius of the cards. @default 0.04 */
  cornerRadius?: number
}
...
void main() {
  ...
  // Aerodynamic Flex: Bends the card trailing the movement direction
  float trailing = direction > 0.0 ? (1.0 - uv.x) : uv.x;
  float flex = pow(trailing, 2.0); 
  
  // Z-axis bowing and X-axis compression
...
void main() {
  ...
  // Apply internal parallax based on X-position
  ...
  // Edge Distortion for light leak & shadow
  float edgeDist = pow(abs(vUv.x - 0.5) * 2.0, 3.0); 
  
  // 2. Cinematic Light Leak
...
function FlexScene({
  ...
  // Dynamically clamp dimensions to keep the carousel beautifully centered
  ...
  // Memoize materials to prevent WebGL memory leaks
  ...
  // Strict GPU Garbage Collection on Unmount
  useEffect(() => {
    return () => {
      geometry.dispose()
  ...
  useEffect(() => {
    ...
    scrollState.current.max =
      (images.length - 1) * spacing + spacing * centerOffset
  ...
  useFrame((_, delta) => {
    ...
    if (groupRef.current) {
      groupRef.current.children.forEach((mesh: any, i) => {
        ...
        // Z-Index sorting: Center item stays on top
 ...
 */
export const FlexCarousel = React.forwardRef<HTMLDivElement, FlexCarouselProps>(
  (
    {
      images,
      className,
    ...
    useGSAP(
      () => {
        ...
        const observer = Observer.create({
          ...
          onWheel: (e) => {
            // Unify horizontal trackpad swipes and vertical scroll wheels
            const delta =
              Math.abs(e.deltaX) > Math.abs(e.deltaY) ? e.deltaX : e.deltaY
            scrollState.current.targetX += delta * scrollSensitivity
          },
          onDrag: (e) => {
            // Drag direction is inverted relative to wheel direction
            const delta =
              Math.abs(e.deltaX) > Math.abs(e.deltaY) ? -e.deltaX : -e.deltaY
            scrollState.current.targetX += delta * scrollSensitivity
          },
    ...
    return (
      ...
      >
        ...
        <div
          ...
          aria-hidden="true"
        >
          {/* Antialiasing is disabled because edges are smoothed via SDF shaders in the fragment */}
          <Canvas
