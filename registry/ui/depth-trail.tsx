...
export default function DepthTrail({
  ...
  // FIX: Added crossOrigin and referrerPolicy to the preloader to prevent Cache collisions and Hotlink blocking
  useEffect(() => {
      ...
      {Array.from({ length: DOM_POOL_SIZE }).map((_, i) => (
        ...
        >
          {imageUrls.map((src, imgIndex) => (
            // FIX: Added crossOrigin and referrerPolicy to the physical DOM nodes
            <img
              key={imgIndex}
              src={src}
              alt="trail"
