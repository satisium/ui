...
export interface CarouselBreakpoint {
  ...
  maxHeight?: number
  /** The minimum height (in pixels) of the smallest, background item */
  minHeight?: number
}
...
export interface AscentCarouselProps {
  ...
  /** Mobile-first default for visible items. @default 6 */
  visibleItems?: number
  /** Mobile-first default for the foreground item's max height in pixels. @default 600 */
  maxHeight?: number
  /** Mobile-first default for the background items' min height in pixels. @default 150 */
  minHeight?: number
    ...
    () => {
      ...
      const update = () => {
        ...
        itemsRef.current.forEach((el, i) => {
          ...
          // Exponential scale mapping creates the curved depth
      ...
      gsap.to(wrapperRef.current, { opacity: 1, duration: 0.5, ease: "power2.out" }) // Prevent FOUC
