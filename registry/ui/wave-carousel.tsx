...
export interface CarouselBreakpoint {
  ...
  visibleItems?: number
  /** The maximum height (in pixels) of the largest, foreground items (the peaks) */
  maxHeight?: number
  /** The minimum height (in pixels) of the smallest, background items (the valleys) */
  minHeight?: number
}
...
export interface WaveCarouselProps {
  ...
  /** Mobile-first default for visible items. @default 9 */
  visibleItems?: number
  /** How many repeating "hills" (sine waves) are visible across the screen at once. @default 2 */
  waves?: number
  /** Mobile-first default for the peak item's max height in pixels. @default 400 */
  maxHeight?: number
  /** Mobile-first default for the valley items' min height in pixels. @default 150 */
  minHeight?: number
  ...
  useGSAP(
    () => {
      ...
      const updateConfig = () => {
        ...
        // Ensure minHeight is strictly smaller than maxHeight to prevent Math.pow rendering bugs
        activeMinH = Math.min(configuredMinH, activeMaxH * 0.8)
        activeMinScale = activeMinH / activeMaxH

        // Calculate the base width required to perfectly fit the items on screen
      ...
      const update = () => {
        ...
        for (let i = 0; i < totalItems; i++) {
          ...
          // THE MAGIC: The sine-wave math shaping the carousel's scale/hills
          const waveMultiplier =
            (Math.cos(normalizedD * Math.PI * 2 * waves) + 1) / 2
    ...
    },
    {
      ...
      dependencies: [
        extendedItems,
        visibleItems,
        waves, // Re-runs layout setup safely if wave frequency changes
        autoMove,
        autoMoveType,
        autoMoveSpeed,
        stepInterval,
