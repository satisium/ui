# Halftone Horizontal Component Context

**Description:** An interactive Canvas 2D component that maps an image into highly detailed, physics-based horizontal halftone strings that react to the cursor.
...
```bash
npx satisium-ui add halftone-horizontal
```
...
| `lines`            | `number`    | `90`    | Number of horizontal lines (rows). Higher = more vertical detail. |
...
| `fallback`         | `ReactNode` | `null`  | Optional fallback UI rendered while the image loads.              |
| `className`        | `string`    | `""`    | Optional standard Tailwind classes for the wrapper.               |

## 3. Core Component Source

**File Path:** `components/ui/halftone-horizontal.tsx`
...
import HalftoneHorizontal from "@/components/ui/halftone-horizontal"
