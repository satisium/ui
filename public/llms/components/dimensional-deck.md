...
**Description:** A high-performance WebGL scroll component for Satis UI. Creates an immersive, 3D stacked deck of images that respond fluidly to scroll momentum anywhere on the container. Utilizes custom GLSL shaders for kinetic RGB splitting and aerodynamic bending. Implements GSAP Observer, strict container binding, and proper ARIA visual masking.
...
| `bendMultiplier`    | `number`   | `0.15`     | Paper aerodynamic flex.     |
| `rgbSplitStrength`  | `number`   | `0.003`    | Kinetic RGB separation.     |
| `stackSpacing`      | `number`   | `1.2`      | Depth spacing multiplier.   |
