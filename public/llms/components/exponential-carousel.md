# Exponential Carousel Component Context

**Description:** A high-performance, mathematics-driven infinite carousel for Satis UI. Uses integration calculus to mathematically pack items side-by-side along an exponential curve, creating a dramatic "hockey stick" perspective using strictly 2D DOM manipulation for maximum FPS. Integrates a GSAP ticker loop and GSAP Observer for flawless touch, drag, and wheel physics. Includes robust GC cleanup and reduced-motion vestibular failsafes that automatically disable infinite scrolling loops.
...
```bash
npx satisium-ui add exponential-carousel
```
...
| :----------------- | :--------------------------- | :------------- | :------------------------------------------------------------ |
| `items`            | `ExponentialCarouselItem[]`  | _Required_     | Image array (`{ id, url, alt }`).                             |
| `visibleItems`     | `number`                     | `10`           | Base visible items.                                           |
| `exponent`         | `number`                     | `4`            | Power curve intensity (1-5).                                  |
| `maxHeight`        | `number`                     | `600`          | Peak height (px).                                             |
| `minHeight`        | `number`                     | `60`           | Tail height (px).                                             |
| `breakpoints`      | `Record<number, Breakpoint>` | `undefined`    | Mobile-first overrides (e.g. `{ 640: { visibleItems: 8 } }`). |
| `autoMove`         | `boolean`                    | `false`        | Enable autoplay.                                              |
| `autoMoveType`     | `"continuous" \| "step"`     | `"continuous"` | Autoplay behavior mode.                                       |
| `autoMoveSpeed`    | `number`                     | `0.005`        | Continuous drift velocity.                                    |
...
**File Path:** `components/ui/exponential-carousel.tsx`
